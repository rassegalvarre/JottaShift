...
public interface IGooglePhotosHttpClient
{
    ...
    Task<Result<TResponse>> GetAsync<TResponse>(string requestUri);

    Task<Result<TResponse>> PostAsync<TRequest, TResponse>(string requestUri, TRequest requestContent);
}
